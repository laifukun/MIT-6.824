...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("***********Reconnected Leader: %d***********", leader1)
	cfg.connect(leader1)
	//cfg.checkOneLeader()
	...
	DPrintf("*********Disconnect Follower: %d*****************", (leader2+1)%servers)
	cfg.disconnect((leader2 + 1) % servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	// if a quorum arises, it should elect a leader.
	DPrintf("*********Connect Follower: %d*******************", (leader2+1)%servers)
...
func TestFailAgree2B(t *testing.T) {
	...
	DPrintf("*********Disconnected Server: %d***************", (leader+1)%servers)
	...
	DPrintf("*********Reconnected Server: %d***************", (leader+1)%servers)
	// re-connect
...
func TestFailNoAgree2B(t *testing.T) {
	...
	DPrintf("********************Disconnected server %d, %d, %d****", (leader+1)%servers, (leader+2)%servers, (leader+3)%servers)
	...
	DPrintf("********************Reconnected server %d, %d, %d****", (leader+1)%servers, (leader+2)%servers, (leader+3)%servers)
	// repair
	cfg.connect((leader + 1) % servers)
...
func TestRejoin2B(t *testing.T) {
	...
	cfg.connect(leader1)
	DPrintf("*************Connect Leader %d**********", leader1)
	cfg.one(104, 2, true)
...
func TestBackup2B(t *testing.T) {
	...
	DPrintf("******************Connected %d %d %d**********", (leader1+2)%servers, (leader1+3)%servers, (leader1+4)%servers)
	...
	DPrintf("***************Disconnected All***********")
	for i := 0; i < servers; i++ {
		cfg.disconnect(i)
	}
	DPrintf("***************Connected %d %d %d***********", (leader1+0)%servers, (leader1+1)%servers, other)
...
func TestFigure82C(t *testing.T) {
	...
	for iters := 0; iters < 1000; iters++ {
		...
		for i := 0; i < servers; i++ {
			if cfg.rafts[i] != nil {
				_, _, ok := cfg.rafts[i].Start(10000 + iters)
				if ok {
					leader = i
				}
			}
